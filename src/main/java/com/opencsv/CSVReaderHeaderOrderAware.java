 ...
 *
 * @author Nico
 * copied from CSVReaderHeaderAware, but uses LinkedHashMap
 */
public class CSVReaderHeaderOrderAware extends CSVReader {
	 ...
	 */
	public CSVReaderHeaderOrderAware(Reader reader) throws IOException {
		super(reader);
		initializeHeader();
	}
	
	/**
	 * Supports non-deprecated constructor from the parent class. Like the
	 * CSVReader this constructor is package scope so only the builder can use
	 * it.
	 *
	 * @param reader The reader to an underlying CSV source
	 ...
	 * @param verifyReader True to verify reader before each read, false
	 * otherwise
	 * @param multilineLimit Allow the user to define the limit to the number of
	 * lines in a multiline record. Less than one means no limit.
	 * @param errorLocale Set the locale for error messages. If null, the
	 * default locale is used.
	 * @param lineValidatorAggregator contains all the custom defined line
	 * validators.
	 * @param rowValidatorAggregator contains all the custom defined row
	 * validators.
	 * @param rowProcessor Custom row processor to run on all columns on a csv
	 * record.
	 * @throws IOException If bad things happen while initializing the header
	 */
	CSVReaderHeaderOrderAware(Reader reader, int skipLines, ICSVParser parser, boolean keepCR, boolean verifyReader,
	...
	/**
	 * Retrieves a specific data element from a line based on the value of the
	 * header.
	 *
	 * @param headerNames Name of the header element whose data we are trying to
	 * find
	 * @return The data element whose position matches that of the header whose
	 * value is passed in. Will return null when there are no more data
	 * elements.
	 * @throws IOException An error occured during the read or there is a
	 * mismatch in the number of data items in a row and the number of header
	 * items
	 ...
	 *
	 * @return A map whose key is the header row of the data file and the values
	 * is the data values. Or null if the line is blank.
	 * @throws IOException An error occurred during the read or there is a
	 * mismatch in the number of data items in a row and the number of header
	 * items.
	 * @throws CsvValidationException If a custom defined validator fails.
	 */
	public LinkedHashMap<String, String> readMap() throws IOException, CsvValidationException {
		String[] strings = readNext();
		if (strings == null) {
			return null;
		}
		...
		LinkedHashMap<String, String> resultMap = new LinkedHashMap<>(headerIndex.size() * 2);
